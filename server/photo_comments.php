...
include "photo_comments_pdo.php";
        ...
        switch ($method) {
            ...
            case "GET":
                ...
                switch ($number_of_url_elements) {
                    case 2:
                        // GET/photo/{photoId}
                        if ($url_elements[1] == "photo_comments") {
                            $response->data = getPhotoComments($pdo, intval($url_elements[2]));
                            $response->status = 200;
                        } else {
                            $response->status = 400;
                            $response->data = null;
                ...
                break;
            case "POST":
                $newPhotoComment = json_decode(file_get_contents("php://input"));

                if(insertPhotoComment($pdo, $newPhotoComment->photoId, $newPhotoComment->username, $newPhotoComment->text)){
                    $response->status = 201;
                } else {
                    $response->status = 400;
                    $response->data = null;
